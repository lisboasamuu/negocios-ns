...
select has_table('public', 'weekday_service_rules', 'weekday service rules exist');
select has_table('public', 'weekday_service_selections', 'weekday service selections exist');
...
select has_function('public', 'save_weekday_service_rule', array['smallint', 'text', 'uuid[]'], 'weekday service admin RPC exists');
...
select ok((select relrowsecurity from pg_class where oid = 'public.weekday_service_rules'::regclass), 'weekday service rules have RLS');
select ok((select relrowsecurity from pg_class where oid = 'public.weekday_service_selections'::regclass), 'weekday selections have RLS');
...
select is((select count(*)::integer from public.weekday_service_rules where mode = 'all'), 7, 'migration preserves behavior with seven all-service rules');
